...
CheckoutService.prototype.createOrder = async function createOrder(data, banner) {
  ...
  try {
    ...
    for (var i = 0; i < Object.keys(cartItems).length; i++) {
      ...
      total = total + parseInt(cartItems[i][keys[6]]);
      listofproducts = `
      <tr class="item">
                <td>
                    ${cartItems[i][keys[1]]}
    ...
    // Update the total of the order to the order table
    const updatetotal = mysql.format(queries.updatetotal,[total,id[0].OrderId]);
    console.log("Total update query:"+updatetotal);
    console.log(await database.query(updatetotal));
